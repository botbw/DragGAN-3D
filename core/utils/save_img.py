...
def save_3d_depth_img(img_depth, path: str='depth_img.jpg'):
    ...
    def get_gauss_filter(in_channels: int, out_channels: int, kernel_size: int, sigma: float):
        ...
        filter = torch.nn.Conv2d(in_channels, out_channels, kernel_size=kernel_size, stride=1, padding=kernel_size // 2, bias=False).cuda()
        filter.weight.data = kernel.view(1, 1, kernel_size, kernel_size).repeat(out_channels, in_channels, 1, 1).cuda()
