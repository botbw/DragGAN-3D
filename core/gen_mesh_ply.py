...
def gen_mesh_ply(filename, eg3d_G, ws, mesh_res=512, max_batch=1000000, device='cuda'):
    samples, voxel_origin, voxel_size = create_samples(
        N=mesh_res, cube_length=eg3d_G.rendering_kwargs['box_warp'] * 1)
    ...
    while head < samples.shape[1]:
        torch.manual_seed(0)
        sigma = eg3d_G.sample_using_ws(samples[:, head:head + max_batch],
                                       transformed_ray_directions_expanded[:, :samples.shape[1] -
                                                                           head],
                                       ws,
